...
export async function ImportXLSContactsService(
  ...
  if (contactList) {
    for (let newContact of contactList) {
      try {
        const response: string | IOnWhatsapp = await CheckContactNumber(newContact.number, companyId);
        const number = typeof response === 'string' ? response : ''; // Garantindo que number seja uma string
