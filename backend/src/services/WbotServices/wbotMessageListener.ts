...
import {isNil, isNull, head, deburr} from "lodash";
...
import {CheckSettings, CheckCompanySetting} from "../../helpers/CheckSettings";
...
const multContactVcard = (arrayContacts) => {
  if (arrayContacts) {
...
export const getBodyMessage = (msg: proto.IWebMessageInfo): string | null => {
  try {
    ...
    const types = {
      ...
      contactsArrayMessage: multContactVcard(msg.message?.contactsArrayMessage?.contacts),
...
const handleOpenAi = async (
  ...
  try {
    ...
    const openaiModel = await CheckCompanySetting(ticket.companyId, "openaiModel", "gpt-3.5-turbo"); // Puxando o modelo de CheckSettings
    ...
    if (openAiIndex === -1) {
      openai = new OpenAI({
        apiKey: prompt.apiKey // Este é o padrão, pode ser omitido
      });
      openai.id = wbot.id;
  ...
  ${prompt.prompt}\n`;
    ...
    for (
      ...
      if (message.mediaType === 'conversation' || message.mediaType === 'extendedTextMessage' || message.mediaType === 'chat') {
        if (message.fromMe) {
...
const verifyMediaMessage = async (
  ...
  if (!msgInstance) {
    ...
    media.filename = deburr(media.filename
      .replace(/^'(.*)'$/, '$1')
    ...
    try {
      ...
      try {
        ...
        console.error('Erro ao criar o diretório ou escrever o arquivo:', error);
      }
    } catch (err) {
...
const searchQueues = (selectedOption, queues) => {
  if (selectedOption)
    return null;
...
const verifyQueue = async (
  ...
  if (queues.length <= 1) {
    console.log('queues.length <= 1')
    let useIntegration = (queues.length == 0 && (whatsapp.integrationId || ticket.integrationId)) || queues[0].integrationId || queues[0].promptId;
    if (useIntegration) {
  ...
  if (queues.length === 1) {
    ...
    if (
      ...
      console.trace(" Iniciei integração dialogflow/n8n. " + ticket.id + ' - ' + ticket.useIntegration)

      const integrations = await ShowQueueIntegrationService(integrationId, companyId);

      await handleMessageIntegration(msg, wbot, integrations, ticket, queueValues)
  ...
  if (choosenQueue) {
    ...
    if (choosenQueue.options.length === 0) {
      ...
      if (queue.outOfHoursMessage !== null && queue.outOfHoursMessage !== "" && !isNil(schedule)) {
        ...
        if (!lastMessage || (lastMessage && lastMessage.time + (1000 * 60 * 5) < new Date().getTime())) {
          ...
          if (now.isBefore(startTime) || now.isAfter(endTime)) {
            var appendText = queue.outOfHoursMessage ? `\u200e ${queue.outOfHoursMessage}\n\n` : "";
            const body = formatBody(`${appendText}*[ # ]* - Voltar ao Menu Principal`, ticket);
      ...
      if (
        ...
        console.trace(" Entrei na integração certa dialogflow/n8n.")

        var integrationId = choosenQueue?.integrationId || ticket.integrationId;
        const integrations = await ShowQueueIntegrationService(integrationId, companyId);

        if (await handleMessageIntegration(msg, wbot, integrations, ticket, queueValues))
          return;
      ...
      if (
        !msg.key.fromMe &&
        !ticket.isGroup &&
        (!isNil(choosenQueue?.promptId) || !isNil(ticket.promptId) || !isNil(whatsapp.promptId))
      ) {

        var promptId = choosenQueue?.promptId || ticket.promptId || whatsapp.promptId;
        await handleOpenAi(msg, wbot, ticket, contact, mediaSent);
  ...
  if (!ticket.contact.disableBot) {
    ...
    if (!isNil(queue) && isNil(ticket.queueOptionId)) {
      ...
      const botText = async () => {
        ...
        let appendText = queue.greetingMessage ? `\u200e${queue.greetingMessage}\n\n` : "";
      ...
      if (queueOptions.length > -1) {
        ...
        const botText = async () => {
          ...
          let appendText = currentOption.message ? `\u200e${currentOption.message}\n\n` : "";
          const textMessage = {
            text: formatBody(`${appendText}${options}`, ticket),
          };
...
export const handleMessageIntegration = async (
  ...
  if (queueIntegration.type === "n8n" || queueIntegration.type === "webhook") {
    if (queueIntegration?.urlN8N) {
      const options = {
        ...
        headers: {
          ...
          "X-N8N-API-KEY": queueIntegration.n8nApiKey || "", // Adicionando a chave da API
        },
        data: msg // Alterado de 'json' para 'data' para compatibilidade com axios
      };
      try {
        const response = await axios(options); // Usando await para melhor tratamento de erros
    ...
    return await typebotListener({ticket, msg, wbot, typebot: queueIntegration, queueValues});
  }
}
...
export const handleMessage = async (
  ...
  try {
    ...
    const ticket = await FindOrCreateTicketService(contact, wbot.id!, unreadMessages, companyId, null, groupContact
      , importing, msg.key.fromMe);
    ...
    try {
      if (!msg.key.fromMe && !importing) {
        ...
        if (bodyMessage?.toUpperCase() === "SAIR" && ticket.status != "closed") {
          ...
          await UpdateTicketService({
            ticketData: {queueId: null, status: "closed", useIntegration: false, integrationId: null},
            ticketId: ticket.id,
            companyId: ticket.companyId,
          });
    ...
    if (hasMedia) {
      ...
      mediaSent = await verifyMediaMessage(msg, ticket, contact, wbot);
    } else {
    ...
    if (
      ...
      const integrations = await ShowQueueIntegrationService(whatsapp.integrationId, companyId);
      console.trace(" Entrei na integração dialogflow/n8n. 1")
